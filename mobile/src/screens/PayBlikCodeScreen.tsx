...
type Props = NativeStackScreenProps<MainStackParamList, 'PayBlikCode'>;
...
export default function PayBlikCodeScreen({ navigation }: Props) {
  ...
  const handleGetQuote = async () => {
    ...
    try {
      ...
      const quoteResult = await getBlikQuote(
        token,
        requestInfo.code,
        selectedChain,
  ...
  const handleSendRealTransaction = async () => {
    ...
    try {
      ...
      // Get recipient address from wallets
      ...
      // Send transaction
      console.log('📤 Sending BLIK payment...');
      ...
      if (token) {
        try {
          await executeBlikPayment(token, {
            ...
            txHash: result.txHash, // Send real txHash to backend
          });
        } catch (error) {
  ...
  return (
    <KeyboardAwareScreen style={styles.container} withSafeArea={true}>
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: theme.colors.text }]}>Pay Blik Code</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <Card style={styles.card}>
        ...
        <View style={[styles.hintBox, { backgroundColor: theme.colors.accent + '10' }]}>
          ...
          <Text style={[styles.hint, { color: theme.colors.textSecondary }]}>
            Enter the 6-digit code from the recipient
          </Text>
        </View>
          ...
          <Card style={styles.card}>
            ...
            {requestInfo.preferredChainId && (
              <View style={[styles.infoRow, { borderBottomColor: theme.colors.border }]}>
                ...
                <Text style={[styles.infoValue, styles.chainValue, { color: theme.colors.primary }]}>
                  {requestInfo.preferredChainId}
                </Text>
              </View>
            )}
